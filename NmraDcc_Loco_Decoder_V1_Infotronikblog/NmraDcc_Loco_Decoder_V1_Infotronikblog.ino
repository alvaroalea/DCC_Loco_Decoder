/* Decodificador para locomotoras H0 con Arduino pro micro (similar a leonardo, con un ATMEGA32 y el driver L9110 (ojo, no admite mas de 12V)
 * Libreria NmraDcc.h 
 * El número por defecto de la locomotora es el 3 
 * La salida PWM en el Arduino Nano es la 9 y 10
 * Las luces van conectadas a los pines 4,5,6,7 y 8
 * La entrada DCC es por el PIN 2
 * La salida DCC (¿opcional?) es por el pin 3
 * 
...
const int right = 9;       //Pin de salida L293
...
int velPwm;            // pin to use as PWM cambia con la direccion.rpbrush
